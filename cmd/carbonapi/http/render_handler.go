...
func getCacheTimeout(logger *zap.Logger, r *http.Request, now32, until32 int64, duration time.Duration, cacheConfig *config.CacheConfig) int32 {
	...
	if duration > cacheConfig.ShortDuration || now32-until32 > 120 {
		return cacheConfig.DefaultTimeoutSec
	}
	// short cache ttl
...
func renderHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(config.Config.TruncateTime) > 0 {
		...
		if useCache {
			responseCacheTimeout = getCacheTimeout(logger, r, 0, 0, 0, &config.Config.ResponseCacheConfig)
			backendCacheTimeout = getCacheTimeout(logger, r, 0, 0, 0, &config.Config.BackendCacheConfig)
		}
	}
